...
description: |
  ...
  log). Activates EVERY session, for any agent. Read and curate it before work,
  keep only recurring high-value guidance, organize by priority-sorted
  categories, and cap each category at top 10 items. The napkin lives at
  `skills/napkin/napkin.md`.
author: Codex
version: 6.0.0
date: 2026-02-21
---
...
**This skill is agent-agnostic and always active. Every session. No trigger
required.** Its activation is declared in the repository `AGENTS.md`, so any
compliant agent (Claude Code, Codex, Cursor, Copilot, Gemini, etc.) applies it.

## Session Start: Read And Curate

First thing, every session — read `skills/napkin/napkin.md` before doing
...
If no napkin exists yet, create one at `skills/napkin/napkin.md`:
