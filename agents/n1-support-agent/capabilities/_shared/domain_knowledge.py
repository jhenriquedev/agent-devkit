#!/usr/bin/env python3
"""Customer support domain knowledge routing helpers for N1 Support Agent."""
...
DOMAIN_KNOWLEDGE_DIR = AGENT_DIR / "knowledge" / "domains" / "customer-support"


def route_customer_symptom(text: str, entities: dict[str, Any] | None = None) -> dict[str, Any]:
    routing = load_json(DOMAIN_KNOWLEDGE_DIR / "symptom-routing.json")
    ...
    contract = load_json(DOMAIN_KNOWLEDGE_DIR / "contracts" / "n1-support-triage-contract.json")
    playbook = load_json(DOMAIN_KNOWLEDGE_DIR / "playbooks" / "global-support-diagnostics.json")
    return {
...
def evaluate_quality_gate(
    *,
    ...
    return {
        ...
        "failures": failures,
        "mustAnswer": load_json(DOMAIN_KNOWLEDGE_DIR / "playbooks" / "global-support-diagnostics.json").get("mustAnswer") or [],
    }
        ...
        gaps.append(
            {
                ...
                "source": "customer-support-symptom-routing",
...
def load_business_rules(knowledge_files: list[str]) -> list[dict[str, Any]]:
    ...
    for ref in knowledge_files:
        if not ref.startswith("rules/"):
            continue
        payload = load_json(DOMAIN_KNOWLEDGE_DIR / ref)
