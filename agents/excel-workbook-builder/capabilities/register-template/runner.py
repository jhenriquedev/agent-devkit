#!/usr/bin/env python3
"""Runner for excel-workbook-builder/register-template."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Register a versioned Excel template")
    ...
    parser.add_argument(
        "--status",
        choices=["draft", "validated", "deprecated", "archived"],
        default="draft",
    )
    ...
    try:
        ...
        if source.suffix.lower() not in {".xlsx", ".xlsm", ".xltx", ".xls"}:
            raise ValueError("template must be .xlsx, .xlsm, .xltx, or .xls")
        ...
        copy_template(source, target_dir / "template.xlsx")
