#!/usr/bin/env python3
"""Runner for generic-agent-builder/plan-generic-agent."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Plan a portable generic agent")
    parser.add_argument("--spec", required=True)
    args = parser.parse_args()

    try:
        result = GenericAgentRepository().plan_generic_agent(spec_path=Path(args.spec))
