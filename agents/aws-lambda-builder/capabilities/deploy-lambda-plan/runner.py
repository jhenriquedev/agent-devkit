#!/usr/bin/env python3
"""Runner for aws-lambda-builder/deploy-lambda-plan."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate an AWS Lambda deploy plan")
    parser.add_argument("--spec", required=True)
    args = parser.parse_args()

    try:
        result = AwsLambdaRepository().deploy_lambda_plan(spec_path=Path(args.spec))
