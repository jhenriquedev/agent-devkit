#!/usr/bin/env python3
"""Tests for the read-card capability runner."""
...
class ReadCardRunnerTest(unittest.TestCase):
    def test_renders_card_analysis_from_fixture(self) -> None:
        ...
        self.assertIn("- Title: Permitir login com SSO", result.stdout)
        self.assertIn("- Current column: Doing", result.stdout)
        self.assertIn("- Created at: 2026-06-20T10:00:00Z", result.stdout)
        ...
        self.assertIn("Acceptance criteria are missing.", result.stdout)
        self.assertIn("Keep collected facts separate from inferences.", result.stdout)
