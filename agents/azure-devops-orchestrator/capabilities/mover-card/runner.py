#!/usr/bin/env python3
"""End-to-end runner for the mover-card capability."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run azure-devops-orchestrator/mover-card")
...
def render_plan(
    ...
    lines = [
        ...
        "## Antes e depois",
        "",
        f"- Estado atual: {value_or_dash(plan['current_state'])}",
        f"- Estado alvo: {value_or_dash(plan['target_state'])}",
        f"- Coluna atual: {value_or_dash(plan['current_column'])}",
        f"- Coluna alvo: {value_or_dash(plan['target_column'])}",
        f"- Risco: {plan['risk']}",
