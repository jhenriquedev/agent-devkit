#!/usr/bin/env python3
"""End-to-end runner for the ler-card capability."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run azure-devops-orchestrator/ler-card")
...
def render_analysis(work_item: dict[str, Any], comments: dict[str, Any] | None = None) -> str:
    ...
    lines = [
        ...
        "## Fatos coletados",
        "",
        f"- Card encontrado no Azure DevOps com estado `{value_or_dash(work_item.get('state'))}`.",
        ...
        *render_attachments(attachments),
        ...
        "## Criterios e escopo",
        "",
        f"- {acceptance_criteria if acceptance_criteria else 'Criterios de aceite ausentes.'}",
        "",
    ...
    if comments_list:
        ...
        lines.append("- Nenhum comentario carregado.")
    ...
    lines.extend(
        [
            ...
            *[f"- {item}" for item in gaps],
            ...
            "## Inferencias",
            "",
            "- Separar fatos coletados de inferencias.",
            "- Validar lacunas com o responsavel do card antes de executar alteracoes.",
            "",
            "## Proximos passos recomendados",
            "",
            "- Confirmar criterios de aceite quando estiverem ausentes ou incompletos.",
            "- Revisar comentarios recentes antes de planejar implementacao.",
        ]
    )
    return "\n".join(lines).rstrip() + "\n"
...
def identify_gaps(work_item: dict[str, Any], comments: list[dict[str, Any]]) -> list[str]:
    ...
    if comments and any("falta" in clean_text(item.get("text")).lower() for item in comments):
        gaps.append("Comentarios indicam pendencias que precisam de refinamento.")
