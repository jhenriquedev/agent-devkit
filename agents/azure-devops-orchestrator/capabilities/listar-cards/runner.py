#!/usr/bin/env python3
"""End-to-end runner for the listar-cards capability."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run azure-devops-orchestrator/listar-cards")
...
def render_list(payload: dict[str, Any], args: argparse.Namespace) -> str:
    ...
    lines = [
        ...
        "## Filtros",
        "",
        f"- Projeto: {value_or_dash(args.project or payload.get('project'))}",
        f"- WIQL: {value_or_dash(args.wiql or payload.get('wiql'))}",
        f"- Estado: {value_or_dash(args.state)}",
        f"- Responsavel: {value_or_dash(args.assigned_to)}",
        f"- Tags: {format_tags(args.tag)}",
        f"- Limite: {value_or_dash(args.limit)}",
        "",
        "## Resultados",
        "",
        "| ID | Tipo | Titulo | Estado | Responsavel | Tags |",
        "|---|---|---|---|---|---|",
    ]
    for item in items:
        lines.append(
    ...
    if not items:
        lines.append("| - | - | Nenhum card encontrado | - | - | - |")

    observations = []
    if not items:
        observations.append("Nenhum resultado retornado para os filtros informados.")
    if payload.get("count") == args.limit and items:
        observations.append("O retorno atingiu o limite informado; podem existir mais cards.")
    observations.append("Use `ler-card` para aprofundar um item especifico.")
