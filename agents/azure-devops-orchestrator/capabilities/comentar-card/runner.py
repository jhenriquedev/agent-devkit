#!/usr/bin/env python3
"""End-to-end runner for the comentar-card capability."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run azure-devops-orchestrator/comentar-card")
    parser.add_argument("--id", type=int, dest="work_item_id")
    parser.add_argument("--project")
    parser.add_argument("--comment")
    parser.add_argument("--comment-intent")
    parser.add_argument("--tone", default="profissional")
...
def build_comment(args: argparse.Namespace) -> str:
    ...
    if not comment and args.comment_intent:
        comment = f"Registro: {clean_text(args.comment_intent)}"
    if not comment:
...
def classify_risk(comment: str) -> tuple[str, str]:
    ...
    if any(term in lowered for term in high_terms):
        return "alto", "Comentario pode envolver dado sensivel ou ambiente critico."
    if any(term in lowered for term in medium_terms):
        return "medio", "Comentario pode registrar compromisso, prazo ou acao operacional."
    return "baixo", "Comentario informativo sem compromisso operacional evidente."
...
def render_comment_plan(
    ...
    status = "executado" if args.execute else "planejado"
    lines = [
        "# Comentario do Card",
        "",
        "## Alvo",
        ...
        f"- Estado atual: {value_or_dash(work_item.get('state'))}",
        "",
        "## Comentario",
        "",
        "```text",
    ...
    if not args.execute:
        lines.extend(["", "## Confirmacao", "", "- Reexecute com `--execute` para publicar."])
    return "\n".join(lines).rstrip() + "\n"
