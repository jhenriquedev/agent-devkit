...
```bash
./ai-devkit run azure-devops-orchestrator atribuir-card --project <project> --id <work-item-id> --assignee pessoa@example.com
```

Para executar a escrita real:

```bash
./ai-devkit run azure-devops-orchestrator atribuir-card --project <project> --id <work-item-id> --assignee pessoa@example.com --execute
```
