#!/usr/bin/env python3
"""End-to-end runner for the atribuir-card capability."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run azure-devops-orchestrator/atribuir-card")
...
def render_plan(
    ...
    lines = [
        ...
        "## Identidade",
        "",
        f"- Candidatos encontrados: {len(users) if not args.skip_identity_lookup else 'lookup ignorado'}",
        f"- Responsavel atual: {value_or_dash(plan['current_assignee'])}",
        f"- Responsavel alvo: {value_or_dash(plan['target_assignee'])}",
        f"- Mudanca real: {value_or_dash(plan['changed'])}",
        f"- Nome alvo: {value_or_dash(target.get('display_name'))}",
        "",
        "## Resultado",
        "",
