#!/usr/bin/env python3
"""End-to-end runner for the update-card-tags capability."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run azure-devops-orchestrator/update-card-tags")
...
def render_plan(
    ...
    lines = [
        ...
        "## Diff",
        "",
        f"- Current tags: {format_tags(plan['current_tags'])}",
        f"- Add: {format_tags(plan['add_tags'])}",
        f"- Remove: {format_tags(plan['remove_tags'])}",
        f"- Final tags: {format_tags(plan['final_tags'])}",
